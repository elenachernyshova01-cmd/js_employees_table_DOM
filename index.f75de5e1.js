...
function getCellValue(row, index) {
    ...
    if (index === 4) return Number(text.replace(/[$,]/g, ""));
    return text.toLowerCase();
}
headers.forEach((header, index)=>{
    header.addEventListener("click", ()=>{
        if (currentSortIndex === index) isAsc = !isAsc;
        else {
        ...
        rows.sort((rowA, rowB)=>{
            ...
            if (typeof valueA === "number") return isAsc ? valueA - valueB : valueB - valueA;
...
tbody.addEventListener("dblclick", (dblClickEvent)=>{
    ...
    if (!cell || document.querySelector(".cell-input")) return;
    ...
    input.addEventListener("keydown", (keyEvent)=>{
        if (keyEvent.key === "Enter") saveCell();
    });
});
